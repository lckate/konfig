...
class TestShell(unittest.TestCase):
    ...
    def test_rm_updates_archive(self):
        # Создаем файл для удаления
        self.shell.touch('tempfile.txt')
        self.assertIn('tempfile.txt', self.shell.ls())

        # Удаляем файл
        result = self.shell.rm('tempfile.txt')
        self.assertIn('File tempfile.txt removed.', result)

        # Проверяем, что файл удален
        self.assertNotIn('tempfile.txt', self.shell.ls())

    def test_rm_existing_file(self):
        self.shell.touch('testfile.txt')
        self.assertIn('testfile.txt', self.shell.ls())

        # Удаляем файл
        result = self.shell.rm('testfile.txt')
        self.assertIn('File testfile.txt removed.', result)
        self.assertNotIn('testfile.txt', self.shell.ls())
